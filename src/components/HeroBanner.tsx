...
import {
  ...
  TrendingUp, TrendingDown, Zap, GraduationCap
} from "lucide-react";
...
const colorConfig: Record<string, { text: string; sparkFrom: string; sparkTo: string }> = {
  purple: {
    text: "hsl(270, 70%, 55%)",
...
const trendData: Record<string, { value: string; up: boolean }> = {
  ...
  "Talabalar soni": { value: "+3.1%", up: true },
};
...
const HeroBanner = ({ cards }: Props) => {
  ...
  return (
    <>
      {/* Title row - spans full grid width */}
      <div className="col-span-2 sm:col-span-3 lg:col-span-5 flex items-center justify-between mb-1">
      ...
      {/* Individual cards - flow in parent grid */}
      {cards.map((card, index) => {
        ...
        return (
          <div
            key={card.label}
            className={`relative bg-card rounded-xl border border-border p-4 cursor-pointer transition-all duration-300 overflow-hidden animate-fade-in opacity-0 ${
